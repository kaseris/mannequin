...
def show_grid_selection_window(_root, figs_dir, pattern: str = None, category=None):
    ...
    opened_imgs_arr = []
    for img_idx in range(len(filtered_images)):
        img_orig = Image.open(filtered_images[img_idx])
        img_resized = img_orig.resize((100, 80),Image.ANTIALIAS)
        ## append tuple of resized image and file with path
        opened_imgs_arr.append( (ImageTk.PhotoImage(img_resized), filtered_images[img_idx]) )
        del img_orig, img_resized

    num_rows_of_images = 3
    n_rows = num_rows_of_images * 3
    n_cols = int(len(filtered_images) / num_rows_of_images)
    print(f"n_rows = {n_rows}\t\tn_cols = {n_cols}\n")

    o_main_window = c_my_wnd_main_window(_root)
    o_main_window.build_frames(n_rows, n_cols, opened_imgs_arr)

def select_images_functionality(root_window):
